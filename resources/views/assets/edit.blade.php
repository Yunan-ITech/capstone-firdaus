...
@section('title', 'Edit Barang - Sistem Manajemen Aset Klinik Firdaus')
            ...
            <form action="{{ route('assets.update', $asset->id) }}" method="POST">
                ...
                <div class="row mb-3">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" id="kode_inventaris" name="kode_inventaris" value="{{ $asset->kode_inventaris }}" readonly>
                    </div>
                    <div class="col-md-6 mb-3">
                        <label for="harga_per_unit" class="form-label">Harga Per Unit <span class="text-danger">*</span></label>
                        <input type="number" class="form-control @error('harga_per_unit') is-invalid @enderror" id="harga_per_unit" name="harga_per_unit" value="{{ old('harga_per_unit', $asset->harga_per_unit) }}" min="0" step="0.01" required>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('jenis_barang_id') is-invalid @enderror" id="jenis_barang_id" name="jenis_barang_id" required>
                            ...
                            @foreach($jenisBarang as $jenis)
                                <option value="{{ $jenis->id }}" {{ old('jenis_barang_id', $asset->jenis_barang_id) == $jenis->id ? 'selected' : '' }}>{{ $jenis->nama_barang }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('kategori_id') is-invalid @enderror" id="kategori_id" name="kategori_id" required>
                            ...
                            @foreach($kategori as $kat)
                                <option value="{{ $kat->id }}" {{ old('kategori_id', $asset->kategori_id) == $kat->id ? 'selected' : '' }}>{{ $kat->nama_kategori }}</option>
                            @endforeach
                        </select>
                        @error('kategori_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="row mb-3">
                    <div class="col-md-4 mb-3">
                        ...
                        <select class="form-select @error('ruangan_id') is-invalid @enderror" id="ruangan_id" name="ruangan_id" required>
                            ...
                            @foreach($ruangan as $r)
                                <option value="{{ $r->id }}" {{ old('ruangan_id', $asset->ruangan_id) == $r->id ? 'selected' : '' }}>{{ $r->nama_ruangan }}</option>
                            @endforeach
                        </select>
                        @error('ruangan_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select class="form-select @error('tahun_id') is-invalid @enderror" id="tahun_id" name="tahun_id" required>
                            ...
                            @foreach($tahun as $t)
                                <option value="{{ $t->id }}" {{ old('tahun_id', $asset->tahun_id) == $t->id ? 'selected' : '' }}>{{ $t->tahun }}</option>
                            @endforeach
                        </select>
                        @error('tahun_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select class="form-select @error('kondisi_id') is-invalid @enderror" id="kondisi_id" name="kondisi_id" required>
                            ...
                            @foreach($kondisi as $k)
                                <option value="{{ $k->id }}" {{ old('kondisi_id', $asset->kondisi_id) == $k->id ? 'selected' : '' }}>{{ $k->nama_kondisi }}</option>
                            @endforeach
                        </select>
                        @error('kondisi_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="row mb-3">
                    <div class="col-md-12 mb-3">
                        ...
                        <textarea class="form-control @error('deskripsi') is-invalid @enderror" id="deskripsi" name="deskripsi" rows="3">{{ old('deskripsi', $asset->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
